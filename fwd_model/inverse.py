    ...
    *,
    ...
    if bf == "DMAS":
      ...
      if signed_dmas:
        ...
        """
        reconstructed_image[(sphere_radius, is_sir, bf, fnumber, apodisation)] = output.squeeze().cpu().numpy().T

    elif bf == "DAS":
        reconstructed_image[(sphere_radius, is_sir, bf, fnumber, apodisation)] = (
            values.sum(dim=-1).squeeze(1).cpu().numpy().T
        )

    return reconstructed_image[(sphere_radius, is_sir, bf, fnumber, apodisation)]
